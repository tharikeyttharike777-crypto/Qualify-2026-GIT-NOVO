...
function exportData(format) {
    ...
    switch (format) {
        case 'excel':
            showNotification(`Exportando ${data.length} registros para Excel...`, 'info');
            break;
        case 'pdf':
            showNotification(`Exportando ${data.length} registros para PDF...`, 'info');
            break;
        case 'csv':
            showNotification(`Exportando ${data.length} registros para CSV...`, 'info');
            break;
    }
    
    // Simular exportação
    setTimeout(() => {
        showNotification(`Exportação ${format.toUpperCase()} concluída!`, 'success');
    }, 2000);
}
