...
export default function ContasPagar() {
  ...
  const filtered = contas.filter(c => {
    ...
    return String(c.descricao || c.fornecedor || '').toLowerCase().includes(q);
  });

  return (
    <div style={{ padding: '10px 0' }}>
      <div className="breadcrumb" style={{ marginBottom: '1.5rem', color: '#666', fontSize: '0.9rem' }}>
        <span>Financeiro</span> <i className="fas fa-chevron-right" style={{ fontSize: '0.75rem', margin: '0 0.5rem' }}></i> <span className="active" style={{ color: '#1565C0', fontWeight: 600 }}>Contas a Pagar</span>
      </div>

      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.8rem', color: '#2c3e50', margin: 0 }}>Contas a Pagar</h1>
      </div>
            ...
            {loading ? (
              <tr>
                <td colSpan="4" style={{ padding: '2.5rem', textAlign: 'center', color: '#64748b' }}>
                  <i className="fas fa-spinner fa-spin" style={{ marginRight: '8px' }}></i> Carregando contas a pagar...
                </td>
              </tr>
            ) : filtered.length === 0 ? (
              <tr>
                <td colSpan="4" style={{ padding: '2.5rem', textAlign: 'center', color: '#64748b' }}>
                  Nenhum conta a pagar encontrada.
                </td>
              </tr>
            ) : (
              filtered.map(c => (
                <tr key={c.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                  <td style={{ padding: '14px 16px', fontWeight: 600, color: '#1e293b' }}>{c.descricao || c.fornecedor}</td>
                  <td style={{ padding: '14px 16px', color: '#475569' }}>{c.vencimento || '-'}</td>
                  <td style={{ padding: '14px 16px', fontWeight: 700, color: '#ef4444' }}>R$ {c.valor || '0,00'}</td>
                  <td style={{ padding: '14px 16px' }}>
                    <span style={{ background: c.status === 'pago' ? '#dcfce7' : '#fee2e2', color: c.status === 'pago' ? '#166534' : '#991b1b', padding: '3px 8px', borderRadius: '10px', fontSize: '0.8rem', fontWeight: 600 }}>
                      {c.status || 'Pendente'}
                    </span>
                  </td>
                </tr>
